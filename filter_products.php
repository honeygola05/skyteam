...
if (isset($_POST['from']) && isset($_POST['to'])) {
    ...
    if (isset($_POST['cat_url']) && $_POST['cat_url'] != '') {
        $cat_url = $_POST['cat_url'];
    ...
    $sql = "SELECT * FROM `product` WHERE `status` = '1' $catFilter AND `discounted_price` BETWEEN $from AND $to";
                ...
                <div class="product-box">
                    <div class="product-imgbox" style="border-top: 1px solid #02d3b3; border-left: 1px solid #02d3b3; border-right: 1px solid #02d3b3;">
                        ...
                        <div class="product-icon icon-inline">
                            <button class="tooltip-top" onclick="addtocart('<?php echo $product['id'] ?>','<?php echo $product['name'] ?>','<?php echo $image[0] ?>','<?php echo $product['discounted_price'] ?>','<?php echo $product['url'] ?>','<?php echo $product['base_price'] ?>')" data-tippy-content="Add to cart">
