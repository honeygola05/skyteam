...
<head>
    ...
    if (isset($_GET['id']) && $_GET['id'] != '') {
        ...
        $res = mysqli_query($con, "select * from inventory where id='$id'");
        ...
        if ($check > 0) {
            ...
            header('location:all-inventary.php');
            die();
        }
    }
    ...
    if (isset($_POST['submit_inventory'])) {
        ...
        $stock_left = get_safe_value($con, $_POST['stock_left']);
        ...
        if ($msg == '') {
            if (isset($_GET['id']) && $_GET['id'] != '') {
                ...
                $update_query = "UPDATE `inventory` SET `product_id`='$product_id',`size`='$size',`stock_left`='$stock_left' WHERE `id`='$id'";

                mysqli_query($con, $update_query);
            } else {
                $insert_query = "INSERT INTO `inventory` (`product_id`, `size`, `stock_left`) VALUES ('$product_id','$size','$stock_left')";
                                        ...
                                        <div class="card-body">
                                            ...
                                            <form method="post" enctype="multipart/form-data">
                                                <div class="theme-form theme-form-2 mega-form">
                                                    ...
                                                    <div class="mb-4 row align-items-center">
                                                        ...
                                                        <div class="col-sm-9">
                                                            <select class="js-example-basic-single w-100" name="product_id" required>
                                                                <?php if ($product_id) {
                                                                    $pro = mysqli_fetch_assoc(mysqli_query($con,"SELECT * FROM `product` WHERE `id` = '$product_id'"));
                                                                    ?>
                                                    ...
                                                    <div class="mb-4 row align-items-center">
                                                        <label class="col-sm-3 col-form-label form-label-title">Select Size</label>
                                                        <div class="col-sm-9">
                                                            <select class="js-example-basic-single w-100" name="size" required>
                                                                <?php if ($size) {
                                                                    $si = mysqli_fetch_assoc(mysqli_query($con,"SELECT * FROM `size` WHERE `id` = '$size'"));
                                                                    ?>
                                                                    <option value="<?php echo $si['id']; ?>" selected><?php echo $si['name']; ?></option>
                                                                <?php } else { ?>
                                                    ...
                                                    <div class="mb-4 row align-items-center">
                                                        <label class="col-sm-3 col-form-label form-label-title">Stock Left</label>
                                                        <div class="form-group col-sm-9">
                                                            <input class="form-control" type="text" name="stock_left" placeholder="Stock Left" value="<?php echo $stock_left ?>">
                                                        </div>
                                                    </div>
                                                ...
                                                <button class="btn btn-solid" name="submit_inventory" type="submit">Submit</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
