...
if (isset($_POST['place_order'])) {
    ...
    $product_measures = $_POST['product_measure'];

    $sql = "INSERT INTO `orders` (`order_id`, `u_id`, `fname`, `lname`, `address`, `city`, `state`, `pincode`, `email`, `phone`, `note`, `subtotal`, `shipping_method`, `final_price`, `product_name`, `product_quantity`, `product_price`,`product_measure`) VALUES ";
    $valueSets = array();
    $productNamesArray = array();
    $productQuantitiesArray = array();
    $productPricesArray = array();
    $productMeasuresArray = array();
    ...
    $productMeasuresJson = json_encode($productMeasuresArray);

    $valueSets[] = "('$order_id','$u_id', '$fname', '$lname', '$address', '$city', '$state', '$pincode', '$email', '$phone', '$note', '$subtotal', '$shipping_method', '$final_price', '$productNamesJson', '$productQuantitiesJson', '$productPricesJson','$productMeasuresJson')";
    ...
    if ($result) {
        ...
        switch ($shipping_method) {
            ...
            case "online":
                ...
                header("Location: thank-you.php?o_id=$order_id");
                break;
            default:
