...
if (isset($_POST['submit'])) {
    ...
    // $material = get_safe_value($con, $_POST['material']);
    ...
    $insert_query = "INSERT INTO `product`(`trending`,`hot_deals`,`availability`, `stock_num`, `url`, `name`, `image`,`image_alt_tag`,`category`,`base_price`,`discounted_price`,`short_description`,`sku`,`description`, `description2`, `meta_title`,`meta_desc`,`meta_url`) 
    VALUES ('$trending','$hot_deals','$availability', '$stock_num', '$url','$name','$product_images','$image_alt_tag','$category','$base_price','$discounted_price','$short_description','$sku','$description', '$description2', '$meta_title','$meta_desc','$meta_url')";
...
if (isset($_POST['update_product_submit'])) {
    ...
    $image_alt_tag = get_safe_value($con, $_POST['image_alt_tag']);
    $category = get_safe_value($con, $_POST['category']);
    // echo $subcategory = get_safe_value($con, $_POST['subcategory']);
    // $brand = get_safe_value($con, $_POST['brand']);
    // $material = get_safe_value($con, $_POST['material']);
    // $link = get_safe_value($con, $_POST['link']);
    $base_price = get_safe_value($con, $_POST['base_price']);
    ...
    // Update the product information in the database
     $update_sql = "UPDATE `product` SET `trending`='$trending',`hot_deals`='$hot_deals',`availability`='$availability', `stock_num`='$stock_num', `url`='$url',`name`='$name', `image`='$product_image', `image_alt_tag`='$image_alt_tag', `category`='$category',`base_price`='$base_price', `discounted_price`='$discounted_price', `short_description`='$short_description', `sku`='$sku', `description`='$description', `description2`='$description2', `meta_title`='$meta_title', `meta_desc`='$meta_desc',`meta_url`='$meta_url' WHERE `id`='$id'";
